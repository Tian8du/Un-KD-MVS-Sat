...
from modules.epmodule import *
from modules.regnet import *
...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, depth_interval=None, view_weights=None):
        ...
        coef = torch.ones((batch, height * width * num_depth, 20), dtype=torch.double).cuda()
        ...
        if view_weights == None:
            ...
            for src_fea, src_proj in zip(src_features, src_projs):
                ...
                print(B, C,h,w, i)
                coef = torch.ones((batch, h * w * num_depth, 20), dtype=torch.double, device=src_fea.device)
                #warpped features
                # src_proj_new = src_proj[:, 0].clone()
                # src_proj_new[:, :3, :4] = torch.matmul(src_proj[:, 1, :3, :3], src_proj[:, 0, :3, :4])
                # ref_proj_new = ref_proj[:, 0].clone()
...
class EPNet(nn.Module):
    def __init__(self, ndepths=[48, 32, 8],min_interval=2.5, depth_interals_ratio=[4, 2, 1], num_groups=[8, 8, 8, 8 ,8], offsetnet_only=False, backbone_only=True):
        ...
        self.feature = FeatExt3(in_channel=1)
        self.ref_feature = FeatExt3_ref(in_channel=1)
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        # feats is the resnet feature, features is the main feature
        # first 48, 96  1/8

        depth_interval = torch.tensor(self.min_interval, dtype=torch.float32,device=imgs.device)
        ...
        for stage_idx in range(self.num_stage):
            ...
            if depth is not None:
                ...
                if self.backbone_only:
                    depth_start = F.interpolate(depth_start.unsqueeze(1), size=(ref_feat.shape[-2], ref_feat.shape[-1]),
                                                mode='bilinear', align_corners=False) - self.ndepths[
                                      stage_idx] * depth_interval * self.depth_interals_ratio[stage_idx] / 2
                else:
                    depth_start = depth_start.unsqueeze(1) - self.ndepths[stage_idx] * depth_interval * \
                                  self.depth_interals_ratio[stage_idx] / 2
                view_weights = F.interpolate(view_weights, size=(ref_feat.shape[-2], ref_feat.shape[-1]),
            ...
            depth_range_samples = get_depth_range_samples(depth_start=depth_start, depth_num=self.ndepths[stage_idx],
                                                          depth_interval=self.depth_interals_ratio[
            ...
            if self.offsetnet_only:
                ...
                outputs_stage = self.DepthNet[stage_idx](features_stage, proj_matrices_stage,
                                                         ...
                                                         depth_interval=self.depth_interals_ratio[
                                                                            stage_idx] * depth_interval,
                                                         num_depth=self.ndepths[stage_idx], view_weights=view_weights)
            else:
                outputs_stage = self.DepthNet[stage_idx](features_stage, proj_matrices_stage,
                                                         depth_values=depth_range_samples,
                                                         depth_interval=self.depth_interals_ratio[
            ...
            outputs_stage['photometric_confidence'] = F.interpolate(
                outputs_stage['photometric_confidence'].unsqueeze(1), [H // 2, W // 2], mode='bilinear',
