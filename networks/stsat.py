...
class ST_SatMVS(nn.Module):
    def __init__(self, geo_model, min_interval=2.5, ndepths=[48, 32, 8],
                 depth_interals_ratio=[4, 2, 1], cr_base_chs=[8, 8, 8], use_qc=False, img_channel=3):
        ...
        self.feature = FeatureNet(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode='fpn',in_channels=self.img_channel) # unet
        self.CoarestFeatureNet = TransformerNet(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode='fpn',in_channels=self.img_channel) #FPN
        ...
        self.GeoFeatureFusionNet = FeatureFusion(convolutional_layer_encoding="z", mask_type="basic", add_origin_feat_flag=True,in_channels=self.img_channel)
