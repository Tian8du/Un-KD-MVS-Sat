...
from modules.ER_module import FeatureNet2, RefineNet2, FeatExt3_ref, Refinement, CostRegNetWithTransformer
from EMVS_Net.Cost_Re import MambaCostRegNet

class OffsetNet(nn.Module):
    def __init__(self, channel):
        super(OffsetNet, self).__init__()
        self.residual_net = Refinement(channel)

    def forward(self, inputs, feat):

        depth_init_res = inputs["depth"]
        depth = self.residual_net(depth_init_res, feat)
        inputs["depth"] = depth
...
def compute_depth_when_train(features, proj_matrices, depth_values, num_depth, cost_regularization, geo_model, use_qc):
    ...
    # regression
    # if depth_values.dim() == 4 and depth_values.shape[2] > 1 and depth_values.shape[3] > 1:
    #     depth_values = depth_values[:, :, 0, 0]  # [B, D]
...
# train+test  CascadeEMVSNet
class CascadeEMVSNet(nn.Module):
    def __init__(self, geo_model, min_interval=2.5, ndepths=[48, 32, 8],
                 depth_interals_ratio=[4, 2, 1], cr_base_chs=[8, 8, 8], use_qc=False, refine=False, Feature='mulbrunch', Costway='3DCNN'):
        super(CascadeEMVSNet, self).__init__()
        ...
        # This is the refine net for refining height
        if self.refine:
            self.refine_network = RefineNet2()
        ...
        # This is the feature extraction
        self.feature = FeatureNet(base_channels=8, num_stage=self.num_stage)

        # This is the Cost Regularization net
        self.cost_regularization = nn.ModuleList([MambaCostRegNet(in_channels=self.feature.out_channels[i],
                                                                     base_channels=self.cr_base_chs[i])
                                                  for i in range(self.num_stage)])
        self.filter_ratio = [4,3,2]
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            # features_stage, proj_matrices_stage, stage_scale
        ...
        # The refine-net combined image feature and boundary will be used to recover boundary information.
        if self.refine and img.size(0) == 1:
            refined_depth = self.refine_network(imgs[:, 0, :, :, :], depth.unsqueeze(0))
            outputs["refined_depth"] = refined_depth
        elif self.refine and img.size(0) != 1:
            refined_depth = self.refine_network(imgs[:, 0, :, :, :], depth.unsqueeze(1))
            outputs["refined_depth"] = refined_depth
...
def compute_depth_when_pEMVS(features, proj_matrices, depth_values, num_depth, cost_regularization, geo_model, use_qc):
    proj_matrices = torch.unbind(proj_matrices, 1)
    assert len(features) == len(proj_matrices), "Different number of images and projection matrices"
...
# pEMVSict  CascadeEMVSNet
class Infer_CascadeEMVSNet(nn.Module):
    def __init__(self, geo_model, min_interval=2.5, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1],
                 cr_base_chs=[8, 8, 8], use_qc=False, refine=True):
        super(Infer_CascadeEMVSNet, self).__init__()
        self.geo_model = geo_model
        assert self.geo_model in ["rpc", "pinhole"]
        self.ndepths = ndepths
        self.depth_interals_ratio = depth_interals_ratio
        ...
        self.feature = FeatureNet2(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode='unet') # unet
        self.cost_regularization = nn.ModuleList([slice_EMVS_Regularization(
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            ...
            outputs_stage = compute_depth_when_pEMVS(features_stage, proj_matrices_stage,
