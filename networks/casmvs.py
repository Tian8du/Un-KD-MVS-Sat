...
class UncertaintyNet(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv0 = ConvBnReLU(self.inplanes, 4*self.inplanes, 3, 1, 1)
        self.conv1 = ConvBnReLU(4*self.inplanes, 8*self.inplanes, 3, 1, 1)
        self.conv2 = ConvBnReLU(8*self.inplanes, self.inplanes, 3, 1, 1)
        self.var_head = nn.Conv2d(self.inplanes, 1, 3, 1, 1)

    def forward(self, x):
        x = self.conv0(x)
        x = self.conv1(x)
        x = self.conv2(x)
        var = F.softplus(self.var_head(x)) + 1e-6      # σ² > 0
        log_var = torch.log(var)                       # ← 关键！存 log σ²
        return log_var.squeeze(1)                      # [B,H,W]


class DepthNet_uncertainty(nn.Module):
    def __init__(self):
        super(DepthNet_uncertainty, self).__init__()

    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, var_reg, prob_volume_init=None, geo_model="rpc", use_qc=False):
        proj_matrices = torch.unbind(proj_matrices, 1)
        assert len(features) == len(proj_matrices), "Different number of images and projection matrices"
        ...
        return {"depth": depth,  "photometric_confidence": photometric_confidence, "features": features,
                        "prob_volume": prob_volume, "depth_values": depth_values, "log_var":log_var}

class CascadeMVSNet(nn.Module):
    def __init__(self, geo_model, refine=False, min_interval=2.5, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1],
                 share_cr=False, grad_method="detach", arch_mode="fpn", cr_base_chs=[8, 8, 8], use_qc=False, img_channel=3):
        ...
        self.feature = FeatureNet(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode=self.arch_mode, in_channels=self.img_channel)
        if self.share_cr:
            self.cost_regularization = CostRegNet(in_channels=self.feature.out_channels, base_channels=8)
            self.var_regression = UncertaintyNet(in_channels=8)
        else:
            self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=self.feature.out_channels[i],
                                                                 base_channels=self.cr_base_chs[i])
                                                      for i in range(self.num_stage)])
            self.var_regression = nn.ModuleList([UncertaintyNet(in_channels=self.feature.out_channels[i])
                                                      for i in range(self.num_stage)])
        if self.refine:
            self.refine_network = RefineNet()
        self.DepthNet = DepthNet_uncertainty()
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            ...
            outputs_stage = self.DepthNet(
                ...
                var_reg=self.var_regression if self.share_cr else self.var_regression[stage_idx], geo_model=self.geo_model, use_qc=self.use_qc)
