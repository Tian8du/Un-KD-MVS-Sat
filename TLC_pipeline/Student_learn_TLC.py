...
from Loss.kd_loss import cas_mvsnet_loss_kl
...
parser.add_argument('--mode', default='distillation', help='train or test', choices=['distillation'])
...
train_dataset = MVSDataset(trainpath, "distillation", args.view_num, ref_view=args.ref_view, use_qc=args.use_qc)
...
def train():
    ...
    # ---------- 2.  LR schedulers ---------------
    warmup_iters = 300                                  # 500 iterations
    ...
    for epoch_idx in range(start_epoch, args.epochs + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            loss, scalar_out, img_out = train_sample_kd(sample)   # forward pass
            ...
            if global_step % args.summary_freq == 0:
                ...
                if loss is None:
                    ...
                    print(f"Iter {batch_idx:04}/{len(TrainImgLoader)} | "
        ...
        with torch.no_grad():
            for b_idx, sample in enumerate(TrainImgLoader):
                loss_val, sc_out, _ = test_sample(sample)
                avg_test.update(sc_out)
                if b_idx % 20 == 0:
                    print(f"[Val] iter {b_idx:03}/{len(TrainImgLoader)} | "
...
def train_sample_kd(sample):
    ...
    valid_pixel_ratio = mask.sum().item() / mask.numel()
    if valid_pixel_ratio < 0.1:
        print(f"[Skip] Batch skipped due to low valid pixel ratio ({valid_pixel_ratio:.4f}).")
