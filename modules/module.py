...
class CostRegNet(nn.Module):
    def __init__(self, in_channels=8, base_channels=8):
    ...
    def forward(self, x):
        ...
        del conv2
        prob = self.prob(conv0 + self.conv11(x))
        del conv0, x
        return prob
...
class TransformerNet(nn.Module):
    def __init__(self,
        ...
        self.conv0 = nn.Sequential(
            Conv2d(self.in_channels, base_channels, 3, 1, padding=1),
            Conv2d(base_channels, base_channels, 3, 1, padding=1),
        )
        '''
        self.conv1 = nn.Sequential(
...
class CostRegNet_Mamba(nn.Module):
    def __init__(self, in_channels, base_channels):
        super(CostRegNet_Mamba, self).__init__()
        self.conv0 = Conv3d(in_channels, base_channels, padding=1)
        ...
        # 加入 Mamba 模块（替代 Transformer）
        self.mamba = Mamba(d_model=base_channels * 8)
    ...
    def forward(self, x):
        ...
        conv2 = self.conv2(self.conv1(conv0))
        conv4 = self.conv4(self.conv3(conv2))
        x = self.conv6(self.conv5(conv4))  # x: [B, C, D, H, W]

        # 加入 Mamba：先 reshape 成序列再还原
        B, C, D, H, W = x.shape
        x_flat = x.view(B, C, -1).transpose(1, 2)     # [B, N, C]
        x_flat = self.mamba(x_flat)                  # [B, N, C]
