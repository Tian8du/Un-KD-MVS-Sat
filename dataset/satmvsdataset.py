...
class MVSDataset(Dataset):
    def __init__(self, data_folder, mode, view_num, ref_view=2, use_qc=False):
        ...
        self.use_qc = use_qc
        assert self.mode in ["train", "val", "test", "pred", "infer", "distillation"]
    ...
    def get_sample(self, idx):
        ...
        # Depth
        # print(new_ndepths)
        depth_min = (valid_depths.min() // 100) * 100
        depth_max = ((valid_depths.max() + 99) // 100) * 100
    ...
    def get_kl_sample(self, idx):
        ...
        # height map, but we name it as depth image to be consistent with that in homography warping
        depth_image = load_pfm((data[2 * self.view_num].replace("height", "height_and_cofi")).replace(".pfm","_depth_est.pfm")).astype(np.float32)
        cof_sigma = load_pfm((data[2 * self.view_num].replace("height", "height_mask")).replace(".pfm","_height_sigma.pfm")).astype(np.float32)
        valid_mask = load_pfm((data[2 * self.view_num].replace("height", "height_mask")).replace(".pfm","_height_mask.pfm")).astype(np.float32)
        mask = valid_mask != -999
        mask = np.float32( mask * 1.0)
        ...
        # Depth
        depth_min = (depth_image.min() // 100) * 100
        depth_max = ((depth_image.max() + 99) // 100) * 100
        depth_values = np.array([depth_min, depth_max], dtype=np.float32)
        ...
        sigma_ms = {
            "stage1": cv2.resize(cof_sigma, (w // 4, h // 4), interpolation=cv2.INTER_NEAREST),
            "stage2": cv2.resize(cof_sigma, (w // 2, h // 2), interpolation=cv2.INTER_NEAREST),
            "stage3": cof_sigma
        }
...
if __name__ == "__main__":
    ...
    train_dataset = MVSDataset(r"E:\Data\WHU_TLC\WHU-TLC\open_dataset_rpc\train", "train", 3, ref_view=0, use_qc=False)
    print("")
